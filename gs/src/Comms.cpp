...
static uint8_t IEEE_HEADER[] = {
    0x08, 0x01, 0x00, 0x00, 0x13, 0x22, 0x33, 0x44, 0x55, 0x66, 0x13, 0x22,
    0x33, 0x44, 0x55, 0x66, 0x13, 0x22, 0x33, 0x44, 0x55, 0x66, 0x10, 0x86,
};
...
static void seal_packet(Comms::TX::Packet &packet, size_t header_offset,
                        uint32_t block_index, uint8_t packet_index) {
  assert(packet.data.size() >= header_offset + sizeof(Comms::TX::Packet));

  Packet_Header &header =
      *reinterpret_cast<Packet_Header *>(packet.data.data() + header_offset);
...
bool Comms::prepare_filter(PCap &pcap) {
  ...
  case DLT_PRISM_HEADER:
    ...
    sprintf(program_src,
            "radio[0x4a:4]==0x11223344 && radio[0x4e:2] == 0x5566");
    break;
  ...
  case DLT_IEEE802_11_RADIO:
    ...
    sprintf(program_src,
            "ether[0x0a:4]==0x11223344 && ether[0x0e:2] == 0x5566");
    break;
  case DLT_EN10MB:
    LOGI("DLT_EN10MB Encap (Standard Ethernet)");
    pcap._80211_header_length = 0;
    sprintf(program_src, "");
    break;
  case DLT_IEEE802_11:
    LOGI("DLT_IEEE802_11 Encap");
    pcap._80211_header_length = 0;
    sprintf(program_src, "");
    break;
  default:
  ...
  if (pcap_compile(pcap.pcap, &program, program_src, 1, 0) == -1) {
    LOGE("Failed to compile program: {} : {}", program_src,
         pcap_geterr(pcap.pcap));
    return false;
  }
  ...
  while (true) {
    {
      ...
      int retval = pcap_next_ex(pcap.pcap, &pcap_packet_header,
    ...
    size_t bytes =
        pcap_packet_header->len - (header_len + pcap._80211_header_length);

    payload += header_len + pcap._80211_header_length;

#ifdef DEBUG_PCAP
    std::cout << "PCAP RX>>";
    std::copy(payload, payload + bytes,
...

    {
      ...
      // find the block
      {
        auto iter = std::lower_bound(
            rx.block_queue.begin(), rx.block_queue.end(), block_index,
            [](RX::Block_ptr const &l, uint32_t index) {
      ...
      memcpy(packet->data.data(), payload + sizeof(Packet_Header),
             bytes - sizeof(Packet_Header));

      // store packet
      if (packet_index >= m_rx_descriptor.coding_k) {
        auto iter = std::lower_bound(
            block->fec_packets.begin(), block->fec_packets.end(), packet_index,
            [](RX::Packet_ptr const &l, uint32_t index) {
              return l->index < index;
            });
        if (iter != block->fec_packets.end() &&
            (*iter)->index == packet_index) {
          // LOGW("Duplicated packet {} from block {} (index {})", packet_index,
          // block_index, block_index * m_coding_k + packet_index);
          return true;
        } else
          block->fec_packets.insert(iter, packet);
      } else {
        auto iter = std::lower_bound(
            block->packets.begin(), block->packets.end(), packet_index,
            [](RX::Packet_ptr const &l, uint32_t index) {
              return l->index < index;
            });
...
#ifdef DEBUG_THROUGHPUT
    {
      ...
      static std::chrono::system_clock::time_point xxx_last_tp =
...
bool Comms::init(RX_Descriptor const &rx_descriptor,
                 TX_Descriptor const &tx_descriptor) {
  ...
  if (m_tx_descriptor.coding_k == 0 ||
    ...
    LOGE("Invalid coding params: {} / {}", m_tx_descriptor.coding_k,
         m_tx_descriptor.coding_n);
    return false;
  }
  ...
  if (m_rx_descriptor.coding_k == 0 ||
    ...
    LOGE("Invalid coding params: {} / {}", m_rx_descriptor.coding_k,
         m_rx_descriptor.coding_n);
    return false;
  }
  ...
  m_impl->rx.streaming_packet_size =
      m_impl->rx.transport_packet_size - m_impl->tx_packet_header_length;
  m_impl->rx.payload_size = m_rx_descriptor.mtu;

  m_impl->tx.transport_packet_size = m_payload_offset + m_tx_descriptor.mtu;
  m_impl->tx.streaming_packet_size =
      m_impl->tx.transport_packet_size - m_impl->tx_packet_header_length;
  ...
  m_impl->rx.block_pool.on_acquire = [this](RX::Block &block) {
    ...
    block.fec_packets.reserve(m_rx_descriptor.coding_n -
                              m_rx_descriptor.coding_k);
  };
  ...
  //    m_impl->pcap = pcap_open_live(m_interface.c_str(), 2048, 1, -1,
  //    pcap_error); if (m_impl->pcap == nullptr)
  //    {
  //        LOGE("Unable to open interface {} in pcap: {}", m_interface,
  //        pcap_error); return false;
  //    }

  ////    if (pcap_setnonblock(m_impl->pcap, 1, pcap_error) < 0)
  ////    {
  ////        LOGE("Error setting {} to nonblocking mode: {}", m_interface,
  /// pcap_error); /        return false; /    }

  //    if (pcap_setdirection(m_impl->pcap, PCAP_D_IN) < 0)
  //    {
  //        LOGE("Error setting {} to IN capture only: {}", m_interface,
  //        pcap_geterr(m_impl->pcap)); return false;
  //    }
  ...
  for (size_t i = 0; i < m_rx_descriptor.interfaces.size(); i++)
    m_impl->rx.threads.push_back(
        std::thread([this, i]() { rx_thread_proc(i); }));
...
#if defined RASPBERRY_PI_XXX
  {
    ...
    if (pthread_setschedparam(m_impl->tx.thread.native_handle(), policy,
                              &param) != 0)
      perror("Cannot set TX thread priority - using normal");
    if (pthread_setschedparam(m_impl->rx.thread.native_handle(), policy,
                              &param) != 0)
      perror("Cannot set TX thread priority - using normal");
  }
#endif
  ...
  while (!m_exit) {
    {
      ...
      if (tx.packet_queue.empty())
        tx.packet_queue_cv.wait(lg, [this, &tx] {
          return tx.packet_queue.empty() == false || m_exit == true;
        });
      ...
      if (packet) {
        seal_packet(*packet, m_packet_header_offset, tx.last_block_index,
    ...
    if (tx.block_packets.size() >= coding_k) {
      if (1) {
        ...
        for (size_t i = 0; i < coding_k; i++)
          tx.fec_src_packet_ptrs[i] =
              tx.block_packets[i]->data.data() + m_payload_offset;
        ...
        for (size_t i = 0; i < fec_count; i++) {
          ...
          tx.fec_dst_packet_ptrs[i] =
              tx.block_fec_packets[i]->data.data() + m_payload_offset;
        }

        // encode
        fec_encode(tx.fec, tx.fec_src_packet_ptrs.data(),
                   tx.fec_dst_packet_ptrs.data(), BLOCK_NUMS + coding_k,
                   coding_n - coding_k, tx.payload_size);

        // seal the result
        for (size_t i = 0; i < fec_count; i++) {
          seal_packet(*tx.block_fec_packets[i], m_packet_header_offset,
                      tx.last_block_index, coding_k + i);
          tx.ready_packet_queue.push_back(
              tx.block_fec_packets[i]); // ready to send
        }
    ...
    while (!tx.ready_packet_queue.empty()) {
      ...
      {
        if (r <= 0) {
          LOGW("Trouble injecting packet: {} / {}: {}", r, isize,
               pcap_geterr(tx.pcap->pcap));
          // result = Result::ERROR;
        }
        if (r > 0 && r != isize) {
          LOGW("Incomplete packet sent: {} / {}", r, isize);
...

    {
...
#ifdef DEBUG_THROUGHPUT
      {
        ...
        if (now - xxx_last_tp >= std::chrono::seconds(1)) {
          float r = std::chrono::duration<float>(now - xxx_last_tp).count();
          LOGI("Sent: {} KB/s / {} KB/s", float(xxx_data) / r / 1024.f,
               float(xxx_real_data) / r / 1024.f);
          xxx_data = 0;
          xxx_real_data = 0;
          xxx_last_tp = now;
        }
...
size_t Comms::get_data_rate() const { return m_data_stats_rate; }

////////////////////////////////////////////////////////////////////////////////////////////

int Comms::get_input_dBm() const { return m_latched_input_dBm; }
...
void Comms::process_rx_packets() {
  ...
  while (!rx.block_queue.empty()) {
    ...
    // try to process consecutive packets before the block is finished to
    // minimize latency
    ...
    if (block->packets.size() + block->fec_packets.size() >= coding_k) {
      ...
      for (size_t i = 0; i < coding_k; i++) {
        if (primary_index < block->packets.size() &&
            i == block->packets[primary_index]->index) {
          rx.fec_src_packet_ptrs[i] =
              block->packets[primary_index]->data.data();
          indices[i] = block->packets[primary_index]->index;
          primary_index++;
        } else {
          rx.fec_src_packet_ptrs[i] =
              block->fec_packets[used_fec_index]->data.data();
          indices[i] = block->fec_packets[used_fec_index]->index;
          used_fec_index++;
        }
      }

      // insert the missing packets, they will be filled with data by the
      // fec_decode below
      size_t fec_index = 0;
      for (size_t i = 0; i < coding_k; i++) {
        if (i >= block->packets.size() || i != block->packets[i]->index) {
          block->packets.insert(block->packets.begin() + i,
                                rx.packet_pool.acquire());
      ...
      lg.unlock(); // not need to hold the mutex locked - give the rx_proc a
                   // chance to get its data in
      fec_decode(rx.fec, rx.fec_src_packet_ptrs.data(),
                 rx.fec_dst_packet_ptrs.data(), indices.data(),
                 rx.payload_size);
    ...
    while ((rx.block_queue.size() > 0 &&
            rx.block_queue.front()->index <
                earliest_block_index) || // if all interfaces received blocks
                                         // bigger that the first in the queue
    ...
    // nothing else to do - we cannot complete nor skip blocks - so wait for
    // more data
    if (!skipped_blocks)
      break;
  }
...
void Comms::process() {
  ...
  if (now - m_data_stats_last_tp >= std::chrono::seconds(1)) {
    ...
    m_data_stats_rate = static_cast<size_t>(
        static_cast<float>(m_data_stats_data_accumulated) / d);
